...
import com.enovlab.yoop.ui.main.mytickets.requested.adapter.RequestedItem.ActiveOnSaleItem
...
import kotlinx.android.synthetic.main.item_requested_active_on_sale.*

class RequestedActiveOnSaleViewHolder(parent: ViewGroup,
                                      private val listener: ((RequestedItem) -> Unit)?,
                                      private val editListener: ((RequestedAdapter.EditData) -> Unit)? = null)
    : RequestedBaseViewHolder(inflateView(R.layout.item_requested_active_on_sale, parent)) {

    fun bind(item: ActiveOnSaleItem) {
        ...
        adapter.listenerEdit = {
            editListener?.invoke(RequestedAdapter.EditData(item.id, MarketplaceType.AUCTION, it.id))
        }
