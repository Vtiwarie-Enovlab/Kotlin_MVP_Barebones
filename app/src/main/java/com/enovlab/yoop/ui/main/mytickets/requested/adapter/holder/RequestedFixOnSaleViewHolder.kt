...
import kotlinx.android.synthetic.main.item_requested_fix_on_sale.*

class RequestedFixOnSaleViewHolder(parent: ViewGroup,
    ...
    : RequestedBaseViewHolder(inflateView(R.layout.item_requested_fix_on_sale, parent)) {

    fun bind(item: RequestedItem.ActionRequiredOnSaleItem) {
        ...
        adapter.listenerFix = {
            fixListener?.invoke(RequestedAdapter.EditData(item.id, MarketplaceType.AUCTION, it.id))
        }
        adapter.listenerClaim = {
            claimListener?.invoke(RequestedAdapter.EditData(item.id, MarketplaceType.AUCTION, it.id))
        }
