...
class OffersPaymentFailedOnSaleViewHolder(parent: ViewGroup, val listener: ((TransactionOfferItem) -> Unit)? = null)
    : BaseViewHolder(inflateView(R.layout.item_requested_transaction_failed, parent)) {

    fun bind(item: TransactionOfferItem.PaymentFailedOnSaleOfferItem, timer: CustomTimer?) {
        val res = itemView.resources

        requested_failed_description.text = res.getString(R.string.my_tickets_requested_on_sale_pending,
