...
class OffersSelectedOnSaleViewHolder(parent: ViewGroup, val listener: ((TransactionOfferItem) -> Unit)? = null)
    : BaseViewHolder(inflateView(R.layout.item_requested_transaction_selected, parent)) {

    fun bind(item: TransactionOfferItem.SelectedOnSaleOfferItem, timer: CustomTimer?) {
        val res = itemView.resources

        requested_selected_description.text = res.getString(R.string.my_tickets_requested_on_sale_pending,
